...
As we plot the ROC curve, we can see that the AUC is equal to 98.27% which is almost a perfect classifier.
...
```{r}
preds_knn = predict(knn_cv,train_set, type = "prob")
train_set %>%
  ggplot() +
...
The train accuracy of our tree is 98.95% with an f1 score of 99% as well with a total of 2 miss-classifications, 1 FP and 1 FN.  
...
99.48% accuracy on training data.
...
The linear kernel performed worse on the training data than the radial kernel, which is expected since it is less flexible and fits the data less well. On the test set however, it gave us a higher level of accuracy.
...
To compare accross our models, we created a function that gets the different relevant metrics and stores them in a dataframe. The metrics we are concerned about are:
1. accuracy
2. f1 score
3. auc
4. error rate
...
As we can see below, the logistic regression model got the greatest AUC value among all models and performed as well as boosted trees and bagged trees, while KNN and SVM with a radial kernel got the smallest values for AUC. 
...
1. We can see from the above visual that LDA has performed better than QDA on the test dataset. We can conclude from this observation that QDA tends to over-fit the training dataset and lead to a decreased ability to generalize on unseen data.
2. The unpruned model was able to learn more features from our dataset than the pruned model, and thus led to a better classification accuracy and a higher AUC value. 
3. SVM with a linear kernel resulted in a better AUC value than the linear one, which pushes us to believe that the decision boundary in our dataset is linear.
