...
```{r, echo=FALSE}
df_0 <- read.csv("./Algerian_forest_fires_dataset_Bejaia.csv")
df_1 <- read.csv("./Algerian_forest_fires_dataset_Sidi_Bel_Abbes.csv")
```
...
df_0[["Region"]] = 0
df_1[["Region"]] = 1
df_1$DC <- as.double(df_1$DC)
df_1$FWI <- as.double(df_1$FWI)
df = full_join(df_0, df_1)
dim(df)
str(df)
head(df_0)
```
